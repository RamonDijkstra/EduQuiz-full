...
def get_data_instances(data_path, eqg_processed, completion_option):
    ...
    - completion_option = 1 -> prompt: context, answer                           completion: question
    - completion_option = 2 -> prompt: context, question                         completion: answer
    - completion_option = 3 -> prompt: context, question, answer possibilities   completion: true answer
    - completion_option = 4 -> prompt: context, question, answer                 completion: distractors
    - completion_option = 5 -> prompt: context                                   completion: question
    - completion_option = 6 -> prompt: context                                   completion: question, answer, and distractors
            ...
            for i in range(len(questions)):
                ...
                if len(eqg_processed) != 0 and compare_question == eqg_processed[0]:
                    ...
                    if completion_option == 1:
                        ...
                        macaw_prompt_text = "$question$" + " ; " + "$answer$ = " + true_answer + " $context$ = " + article
                        ...
                        for option in options[i]:
                            gpt_prompt_text += 'Answer option: ' + option + '\n'
                        gpt_prompt_text += '\n###\n\n'
                        gpt_completion = ' ' + 'True answer: ' + true_answer + '\n###'
                        
                        macaw_prompt_text = "$answer$" + " ; " + "$question$ = " + question + " $mcoptions$ = (A) " + options[i][0] + " (B) " + \
                                            options[i][1] + " (C) " + options[i][2] + " (D) " + options[i][3] + " $context$ = " + article
                        macaw_completion = true_answer
                        ...
                        assert False, 'Quiz completion options are: 1,2,3,4,5,6'
...
def write_files():
    # Have all the 6 different completion options
    completion_options = [1,2,3,4,5,6]
